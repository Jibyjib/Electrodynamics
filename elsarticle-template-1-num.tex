...
I skipped explaining mathematics because I want to write a completely separate set of notes on that later. I also skipped the intro to relativity because I believe that also belongs in a different set of notes. 

The one of the main notations that I differ from Griffiths on is the symbol for the scalar potential. I use $\phi$, while he uses V. I looked at Jackson and he uses $\Phi$, while Pauli uses $\psi$. It seems there is no strict standard for the variable.

I think what these notes are mainly lacking at the current time are figures, examples, and derivations. I will try to introduce each of these things over time. I hope everyone can benefit from what I have done here so far though. Good luck reading!

\section{Basic Electrostatics}

In electrostatics we have some bread and butter equations. First there will be Coulomb's law which will allow us to do the basic problems involving point charges. The next big equations we will run into is Gauss's law, Laplace's equation, and Poisson's equation. From there we will try to establish boundary conditions as well as explore work and energy for electrostatics. We will end this chapter off which talk about some rudimentary discussion of conductors and capacitors.
...
\begin{equation}
    \b{E}(\b{r}) = \k \sum_{i=1}^n \frac{q_i}{\r^2} \hr
\end{equation}
...
\begin{equation}
    \b{E}(\b{r}) = \k \int \frac{1}{\r^2}\hr \hspace{3.5pt} dq.
\end{equation}

That dq though can be a tricky thing to establish. Instead we would normally know what the charges are evenly distributed, and given what the distribution is. For one dimensional case you will be given the linear charge distribution $\lambda$, for two dimensions the surface charge density $\sigma$, and for three dimensions the volume charge density $\rho$. Writing these in terms of differentials,

\begin{equation}
\begin{split}
    & dq = \frac{dq}{d\ell}d\ell = \lambda d\ell, \\
    & dq = \frac{dq}{da}da = \sigma da, \\
    & dq = \frac{dq}{d\tau}d\tau = \rho d\tau.
\end{split}
\end{equation}
...
\begin{equation}
    \phi(\b{r}) = \k \9 \frac{q}{\r_+}+\frac{-q}{\r_-} \0 .
\end{equation}
...
\begin{equation}
    \phi(\b{r}) = \k \sum_{n=0}^\infty \frac{1}{r^{n+1}} \int (r')^n P_n(\cos \alpha ) \rho(\b{r}')d\tau'.
\end{equation}
...
\begin{equation}
    \b{B}(\b{r}) = \m \int \frac{\b{I} \x \hr }{\r^2}d\ell' = \m I \int \frac{d\bell \x \hr}{\r^2}.
\end{equation}
...
\begin{equation}
    \curl \9 \frac{1}{\mu_0}\b B - \b M \0 = \b J_f.
\end{equation}
...
The second most prevalent gauge is called the Lorenz gauge, and is useful for relativistic problems because it preserves Lorentz invariance. The Lorenz gauge is stated as 

\begin{equation}
    \div \b A = - \frac{1}{c^2}\pd{\phi}{t}.
\end{equation}

We saw this gauge in the relativity section before, and so I will not talk to much more about it here. Using the Lorenz gauge though boils down to just solving the inhomogeneous wave equation. We will be assuming the Lorenz gauge from here on out.

\subsection{Retarded Potentials}

Because information takes time to travel from one place to another, namely it travels at the speed of light, having potentials and fields respond instantaneously to the change in charges and currents is inherently wrong. Therefore lets define a retarded time that takes into account the travel of information

\begin{equation}
    t_r \= t - \frac{\r}{c}.
\end{equation}

If we just plug these equations into the formulas for the scalar and vector potentials then we get what is called the retarded potentials. Applying this concepts to the fields gets you the wrong answer. Checking that just plugging in the retarded time into the potentials we find that we could have defined an advanced time 

\begin{equation}
    t_a \= t + \frac{\r}{c},
\end{equation}

and it would have worked as well. The problem with using advanced time is that it breaks causality which is a big no no.

Now knowing that these potentials work we can just plug them into the equations for the fields and get the solutions. In doing so we run into what is called Jefimenko's equations,

\begin{equation}
    \b E(\b r,t) = \k \int \9 \frac{\rho(\b r', t_r}{\r^2}\hr + \frac{\dot{\rho}(\b r', t_r)}{c\r}\hr - \frac{\dot{\b J}(\b r', t_r)}{c^2 \r} \0 d\tau',
\end{equation}

and

\begin{equation}
    \b B(\b r,t) = \m \int \9 \frac{\b J(\b r', t_r)}{\r^2} + \frac{\dot{\b J}(\b r', t_r}{c\r} \0 \x \hr \hspace{3.5pt} d\tau'.
\end{equation}

\subsection{Point Charges}

What are the potentials of a moving point charge? Well the answers are the Li\'enard-Wiechert potentials. We could try to introduce some sort of handwavey geometrical argument to derive these, but instead I will try to derive them from a distributional point of view.\footnote{When I say "I will," I mean I will do that later when I have more time.}

\begin{equation}
    \phi(\b r, t) = \k \frac{qc}{\r c - \br \. \b v},
\end{equation}

and

\begin{equation}
    \b A(\b r, t) = \frac{\b v}{c^2} \phi(\b r, t).
\end{equation}

We can use the Li\'enard-Wiechert potentials and solve for the fields directly. The results are

\begin{equation}
    \b E(\b r,t) \k \frac{q \r}{(\br \. \b u)^3}[(c^2-v^2)\b u + \r \x (\b u \x \b a)],
\end{equation}

and

\begin{equation}
    \b B(\b r, t) = \frac{\hr}{c} \x \b E(\b r, t).
\end{equation}

Lets examine the electric field a little closer. The first time is called the velocity field, and it's main use is swapping the energy between the field and the particles. The second term is called the acceleration term and it introduces a way for energy to be loss from the system, i.e. radiation.
...
\begin{equation}
    \phi(\b r,t) = \k \int \frac{\rho(\b r', t- \r/c)}{\r}d\tau',
\end{equation}
...
\begin{equation}
    \b E(\b r,t) = \k \frac{\r}{(\br \. \b u)^3}[(c^2-v^2)\b u + \br \x ( \b u \x \b a)],
\end{equation}

where $\b u = c \hr - \b v$. If we recall from the potentials and fields chapter, we will remember this field is broken up into the velocity and acceleration fields. The acceleration field is the one that will contribute to the radiation in the end and so is sometimes referred to as the radiation field. The reason for this is the velocity field carries around the energy, but does not take it to infinity. 

The magnetic field created by this charge is

\begin{equation}
    \b B(\b r, t) = \frac{1}{c} \hr \x \b E(\b r, t).
\end{equation}

and thus we are able to carry out the normal process of figuring out the Poynting and calculating the radiation you would end back up at the Larmor formula if you are travelling at a non-relativistic speed. What if this was at a relativistic speed though? The relativistic equation to the Larmor formula is called Li\'enard's generalization and is

\begin{equation}
    P = \frac{\mu_0 q^2 \gamma^6}{6 \pi c} \9 a^2 - \frac{|\b v \x \b a^2|}{c^2} \0.
\end{equation}

This is talking about the classical way of interpreting bremsstrahlung.
