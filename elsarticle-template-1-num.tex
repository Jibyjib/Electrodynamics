...
\def\rcurs{{\mbox{$\resizebox{.09in}{.08in}{\includegraphics[trim= 1em 0 14em 0,clip]{images/ScriptR.pdf}}$}}}
...
\begin{equation}
    \vec{F} = \k\frac{qQ}{\rcurs^2}\hat{\rcurs}.
\end{equation}
...
\begin{equation}
    \vec{E}(\vec{r}) = \frac{1}{4 \pi \epsilon_0} \sum_{i=1}^n \frac{q_i}{\rcurs^2} \hat{\rcurs}
\end{equation}
...
\begin{equation}
    \vec{E}(\vec{r}) = \k \int \frac{1}{\rcurs^2}\hat{\rcurs}dq.
\end{equation}
...
\begin{equation}
    \vec{E}(z) = \k \frac{2\lambda}{z} \hat{z}.
\end{equation}
...
\begin{equation}
    \phi(\vec{r}) = \k \int \frac{\rho(\vec{r'})}{\rcurs}d\tau'.
\end{equation}
...
\begin{equation}
    \phi(\vec{r}) = \k\oint_S \frac{\sigma_b}{\rcurs}da' + \k \int_v \frac{\rho_b}{\rcurs}d\tau'.
\end{equation}
...
\begin{equation}
    \vec{K} \equiv \frac{d\vec{I}}{d\ell_\bot} = \sigma \vec{v},
\end{equation}

and for three dimensional currents,

\begin{equation}
    \vec{J} \equiv \frac{d\vec{I}}{da_\bot} =\rho \vec{v}.
\end{equation}
...
\begin{equation}
    \vec{B}(\vec{r}) = \m \int \frac{\vec{I} \x \hat{\rcurs}}{\rcurs^2}dl' = \m I \int \frac{d\vec{l'} \x \hat{\rcurs}}{\rcurs^2}.
\end{equation}

Here I have introduced a new constant, $\mu_0$, which is called the permeability of free space. Unless you are trying to be more precise than $10^{-16}$, you can simply use $\mu_0 = 4\pi \x 10^{-7}$.
...
For electrostatics we took advantage of the fact that $\nabla \x \vec{E} = 0$ to define a scalar potential. Now for magnetostatics we will take advantage of $\nabla \. \vec{B} = 0$ to define a vector potential. Vector calculus identities allow us to define a vector such that,
...
\begin{equation}
    \vec{A}(\vec{r)} = \m \int \frac{\vec{J(\vec{r'})}}{\rcurs}d\tau'.
\end{equation}
